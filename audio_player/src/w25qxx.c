...
esp_err_t write_audio_data_to_flash(w25qxx_t *dev, uint32_t start_addr, uint8_t *audio_data, size_t data_len) {
    ...
    for (size_t sector = 0; sector < total_sectors; sector++) {
	    ...
	    for (size_t offset = 0; offset < sector_size && data_len > 0; offset+=page_size) {
	        uint32_t page_addr = sector_addr + offset;
	        size_t bytes_to_write = (data_len > page_size) ? page_size : data_len;
	        if (w25qxx_pageWrite(dev, sector,page_addr % sector_size, audio_data , bytes_to_write) != bytes_to_write) {
	            ESP_LOGE(TAG, "Failed to write data at address ");
	            return ESP_FAIL;
	        }
	        else{
				ESP_LOGE(TAG, "Data written successfully at page %d of sector %d ", offset/ page_size, sector);
			}
	        ...
	        uint8_t buffer[256];
            memset(buffer, 0, 256);
	        int len =  w25qxx_read(dev, page_addr, buffer, 256);
			if (len != 256) {
			ESP_LOGE(TAG, "read fail");
			while(1) { vTaskDelay(1); }
			}
			ESP_LOGI(TAG, "Read Data: addr = %" PRIu32 "\n", page_addr);	
     		dump(buffer, 256);
	        
            }

    	}
		ESP_LOGI(TAG, "Successfully wrote audio data to flash memory");
